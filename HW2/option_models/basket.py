...
def basket_price_mc(
    ...
    if( bsm ) :
        ...
        '''
        prices=forward[:,None]*np.exp(-1/2*texp*np.diag(cov_m)[:,None]+np.sqrt(texp)*chol_m@znorm_m)    
        pass
    else:
...
def basket_price_norm_analytic(
    ...
    sigma_N_J=(vol*spot)[:,None]
    vol=np.sqrt((weights*sigma_N_J).T@cor_m@(weights*sigma_N_J))
    
    spot=np.sum(weights.T*spot)
    
    price=normal_formula(strike, spot, vol, texp, intr=0.0, divr=0.0, cp_sign=1)
